...
public class BlockSummerCore extends Block {
    private final String name = "summer_core";
    public BlockSummerCore() {
        super(Material.IRON);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        WorldSeason.setWorldState(2);
        WorldSeason.setSeason(Season.SUMMER);

        MinecraftServer mc = FMLCommonHandler.instance().getMinecraftServerInstance();
        mc.getPlayerList().sendMessage(new TextComponentString(I18n.format("message.summer_core_block.active")));
