...
public class OverClientSounds {
	...
	public static void playSound(String uuid, String soundMethod, double posX, double posY, double posZ) {
		...
		if (player.getCachedUniqueIdString().equals(uuid) && player.world.isRemote) {
			...
			else if (soundMethod.equals("InsanityAncientSoundLoud")) { playInsanityAncientSoundLoud(world, pos); }
		}
	}
	...
	// Louder version of insanity ambience.
	private static void playInsanityAncientSoundLoud(WorldClient world, BlockPos pos) {

		world.playSound(pos, SRPSounds.ANCIENT, SoundCategory.AMBIENT, 0.5f, 0.2f, false);
	}
}