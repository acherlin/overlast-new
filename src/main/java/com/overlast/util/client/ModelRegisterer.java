...
import com.overlast.lib.ModExtraItems;
...
public class ModelRegisterer {
	...
	public void registerModels(ModelRegistryEvent event) {
		...
		registerModelsItem(event,ModItems.ITEMS);
		if (Loader.isModLoaded("spartanshields")) {
			registerModelsItem(event, ModExtraItems.ITEMS);
		}
	...
	public void registerModelsItem(ModelRegistryEvent event,Item[] ITEMS) {
		for (Item item : ITEMS) {
			// If there are subitems (items with metadata), create a list of them and register those models separately.
			if (item.getHasSubtypes()) {

				NonNullList<ItemStack> subItems = NonNullList.create();
				item.getSubItems(TabOverLast.TAB_overlast, subItems);
