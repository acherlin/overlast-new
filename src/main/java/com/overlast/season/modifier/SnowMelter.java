...
public class SnowMelter {
	 ...
	 *  8 ------------------------>
	 * 
	 * Each row is 16 chunks. 每行是16个区块。
	 */
	
	// Yay pseudorandom 伪随机万岁
	private Random rand = new Random();
	
	// What chunks to go through? 要经过哪些环节？
	private int setOffset() {
		
		// Offset 偏移量Z
		int offsetZ = 0;
		
		// Grab a direction ID 抓住一个方向的ID
	...
	public void melt(World world, EntityPlayer player, int daysIntoSeason) {
		...
		while (chunkNum < chunkLimit && chunk != null) {
			...
			// 将有一个随机的机会来清除某个大块中的雪层或冰块。
			// 如果运气好的话，我们会选择一个随机的冰块来清除雪。
			// 另外，如果该生物群落的温度仍然低到足以有雪，就不要费心做这个。
	...
	//到了夏天，任何非雪地生物群落都不应该有雪。仅限夏季
	public void meltCompletely(int startX, int startZ, World world) {
		
		/* 我们会这样做，以避免浪费宝贵的CPU。因为每次有人进入一个大块时都会发生这种情况。
